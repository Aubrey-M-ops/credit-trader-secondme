...
- **Correct**: `${SECONDME_API_BASE_URL}/api/user/info`

According to `.secondme/state.json`, the user info endpoint is `/api/user/info`, not `/api/secondme/user/info`.
...
2. ✅ Returns API key (starts with "ct_")
...
1. ✅ **API Endpoint**: Corrected path to `/api/user/info`
...
6. Fetch user info from: /api/user/info ✅
